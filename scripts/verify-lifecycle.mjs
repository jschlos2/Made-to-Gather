...
const draftEvent = events.find((item) => item.lifecycle.status === 'draft');
...
assert.ok(draftEvent, 'At least one draft event is required for lifecycle verification.');
assert.equal(draftEvent.lifecycle.rsvpOpen, false);
assert.equal(draftEvent.lifecycle.photoUploadsOpen, false);
assert.equal(draftEvent.photos.uploadsEnabled, false);
assert.equal(draftEvent.photos.galleryEnabled, false);
assert.match(draftEvent.date.monthDay, /TBD/);
assert.match(draftEvent.time, /TBD/);
...
const draftResponse = await eventGuard({ env: {}, request: new Request(`https://example.com/events/${draftEvent.slug}/`), next: async () => new Response('should not render') });
